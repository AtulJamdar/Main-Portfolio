...
// FIX 1: Reduced-motion-safe variants — still fade in, just no scale/y movement
const pillVariantsMotion = {
...
// ─── Pill component (memoized) ───────────────────────────────────────────────

// FIX 2: Extracted into a memoized component — was copy-pasted 3 times (once
// per row). memo() prevents re-renders unless name/slug/color actually change.
const TechPill = memo(function TechPill({
  ...
  return (
    <motion.div
      variants={reduceMotion ? pillVariantsReduced : pillVariantsMotion}
      // FIX 3: whileHover scale disabled for reduced-motion users
      whileHover={
        reduceMotion
          ? { borderColor: "#a855f7", backgroundColor: "rgba(147, 51, 234, 0.1)" }
          : { scale: 1.05, borderColor: "#a855f7", backgroundColor: "rgba(147, 51, 234, 0.1)" }
      }
    ...
    >
      <span className="w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center">
        {/* FIX 4: External CDN icons — next/image requires domain allowlist so
            we keep <img> here, but loading="lazy" + explicit width/height are
            already correct. Added decoding="async" so decoding happens off the
            main thread and doesn't block paint. */}
        <img
          alt={name}
          loading="lazy"
          decoding="async"
...
export default function TechStack() {
  ...
  // FIX 5: Respect OS reduced-motion preference
  ...
  // FIX 6: When reduced-motion, keep pathLength static at 1 (fully drawn)
  // so the triangle is visible without animation.
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4 relative z-10">
        ...
        {/* FIX 7: Single pill row renderer — eliminates the 3x copy-paste */}
        <motion.div
        ...
        >
          {ROWS.map((row, rowIdx) => (
            <div
              key={rowIdx}
              className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2 sm:gap-3 w-full sm:w-auto justify-center"
            >
              {row.map((tech) => (
                <TechPill
                  key={tech.name}
